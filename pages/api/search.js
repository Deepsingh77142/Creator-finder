export default async function handler(req, res) {
  ...
  try {
    const searchUrl = `https://www.googleapis.com/youtube/v3/search?part=snippet&type=channel&maxResults=25&q=${encodeURIComponent(niche)}&key=${apiKey}`;
    ...
    const statsUrl = `https://www.googleapis.com/youtube/v3/channels?part=snippet,statistics&id=${channelIds.join(',')}&key=${apiKey}`;
    const statsRes = await fetch(statsUrl);
    const statsData = await statsRes.json();
    if (statsData.error) throw new Error(statsData.error.message);

    const filtered = statsData.items.filter(ch => {
      const subs = parseInt(ch.statistics.subscriberCount || 0);
      return subs >= min && subs <= max;
    });

    const results = filtered.map(ch => {
